...
class VectorStore:
    ...
    def build(self, docs):
        ...
        self.embeddings = self.model.encode(
            docs, convert_to_tensor=True, show_progress_bar=True, device=self.device
        )

    def add_documents(self, new_docs):
        """Add new documents and update embeddings"""
        new_embeddings = self.model.encode(
            new_docs, convert_to_tensor=True, show_progress_bar=True, device=self.device
        )
    ...
    def save(self, emb_path="data/embeddings.pt", doc_path="data/documents.pt"):
        """Save embeddings + documents to disk safely"""
        # Ensure directories exist
        os.makedirs(os.path.dirname(emb_path), exist_ok=True)
        os.makedirs(os.path.dirname(doc_path), exist_ok=True)

        torch.save(self.embeddings, emb_path)
        torch.save(self.documents, doc_path)

    def load(self, emb_path="data/embeddings.pt", doc_path="data/documents.pt"):
        """Load embeddings + documents from disk"""
        self.embeddings = torch.load(emb_path, map_location=self.device)
        self.documents = torch.load(doc_path)
